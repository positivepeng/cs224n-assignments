...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        PackedSequence = nn.utils.rnn.pack_padded_sequence(X, source_lengths)
        # output: (seq_len, batch, num_directions * hidden_size):
        # h_n: (num_layers * num_directions, batch, hidden_size)
        # c_n: (num_layers * num_directions, batch, hidden_size)
        enc_hiddens, (hn, cn) = self.encoder(PackedSequence)
        (enc_hiddens, lengths) = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):  # iterate element in y  
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # enc_hiddens_proj : (b, src_len, h) ==  W_{attProj}[2h->h] h^enc (batched)[b src_len 2h].
        # bmm : bxnxm  bxmxp --> bxnxp
        # bmm((b,scr_len,h), (b,h,1)) --> (b,src_len,1) -> squeeze -> (b,src_len)
        e_t = torch.squeeze(
